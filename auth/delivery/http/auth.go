...
func (a *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if auth == true {
		...
		logs.LogError(logs.Logger, "auth_http", "Email", err, "User is already logged in")
		return
	}
	...
	if err != nil {
		...
		logs.LogError(logs.Logger, "auth_http", "Email", err, "Failed to decode json from body")
		return
	}
	logs.Logger.Debug("Email credentials:", credentials)
	...
	if err = checkCredentials(credentials); err != nil {
		...
		logs.LogError(logs.Logger, "auth_http", "Email", err, "Credentials are incorrect")
		return
	}
	...
	if err != nil {
		domain.WriteError(w, err.Error(), domain.GetHttpStatusCode(err))
		logs.LogError(logs.Logger, "auth_http", "Email", err, "Failed to login")
		return
	}
	logs.Logger.Debug("Email: session:", session)
